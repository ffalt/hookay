...
var Tasker = function (site, notify) {
	...
	var buildJekyll = function (task, cb) {
		...
		exec('bundle ' + params + 'exec jekyll build -d ' + dest_dir, {cwd: jekyll_dir}, function (err, stdout, stderr) {
			var result = '';
	...
	var runJekyll = function (task, cb) {
		buildJekyll(task, function (err, result) {
				if (!err) {
					...
					return exec('bundle ' + params + '--path ' + jekyll_dir + '/.gem', {cwd: jekyll_dir}, function (err, stdout, stderr) {
							if (stderr && stderr.toString().length > 0) {
								err = stderr.toString();
								emit('error', task, err);
								return cb(err);
