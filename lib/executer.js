...
var Tasker = function (site, notify) {
  ...
  me.runTask = function (task, cb) {
    me.tasks.push(task);
    emit('start', task);
    if (site.engine && site.engine === "hugo") {
      async.series([
        function (then) {
          getGit(task, then);
        },
  ...
  var updateGit = function (task, branch, git, git_dir, cb) {
  ...
  var resetGit = function (task, branch, git, git_dir, cb) {
    emit('resetting', task, {path: git_dir, repro: site.repro});
    git.reset(['--hard', 'origin/' + branch], function (err) {
      if (err) {
	cb(err);
      } else {
	emit('resetted', task);
  ...
  var getGit = function (task, cb) {
    ...
    if (!fs.existsSync(git_dir)) {
      var err = 'directory ' + git_dir + ' must exist';
      emit('error', task, err);
      return cb(err);
    }
	...
	updateGit(task, branch, git, git_dir, function (err) {
	  if (err) {
	    emit('warning', task, err);
	    resetGit(task, branch, git, git_dir, function (err) {
	      if (err) {
		emit('error', task, err);
		return cb(err);
	      }
  ...
  var buildJekyll = function (task, cb) {
    ...
    var exec_options = {cwd: jekyll_dir};
    if (site.env) {
      var envCopy = {};
      var env = process.env;
	...
	envCopy[key] = env[key];
      ...
      Object.keys(site.env).forEach(function (key) {
	envCopy[key] = site.env[key];
      });
      exec_options.env = envCopy;
    }
    exec(params + 'bundle exec jekyll build -d ' + dest_dir, exec_options, function (err, stdout, stderr) {
      var result = '';
  ...
  var runJekyll = function (task, cb) {
	...
	return exec(cmd, exec_options, function (err, stdout, stderr) {
	  if (stderr && stderr.toString().length > 0) {
	    err = stderr.toString();
	    emit('error', task, err);
	    return cb(err);
  ...
  var buildHugo = function (task, cb) {
    var hugo_dir = path.resolve(site.build_path);
    var dest_dir = path.resolve(site.build_path, '_site'); // set it to the same as jekyll would
    emit('building (hugo)', task, {path: dest_dir, dest: dest_dir});

    exec('bin/hugo/hugo -d ' + dest_dir, function (err, stdout, stderr) {
      var result = '';
      if (stdout && stdout.toString().length > 0) {
	result += stdout.toString();
      }
  ...
  var runHugo = function (task, cb) {
    buildHugo(task, function (err, result) {
      if (!err) {
	emit('build (hugo)', task, result);
	return cb();

      } else if ((err.indexOf('no such file or directory') >= 0)) {
        // should check if there is a package.json
        // but first step assume there is one there

	var hugo_dir = path.resolve(site.build_path);
	emit('installing', task, {path: hugo_dir});

	var cmd = 'npm install ' + hugo_dir + '/node_modules';
	var exec_options = {cwd: hugo_dir};
	return exec(cmd, exec_options, function (err, stdout, stderr) {
	  if (stderr && stderr.toString().length > 0) {
	    err = stderr.toString();
	    emit('error', task, err);
	  ...
	  emit('installed (hugo)', task, result);

	  buildHugo(task, function (err, result) {
	    if (!err)
	      emit('build (hugo)', task, result);
	    else
	      emit('error (hugo)', task, err);
	    return cb(err);
	  });
	});
