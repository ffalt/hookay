...
var Tasker = function (site, notify) {
	...
	var getGit = function (task, cb) {
		...
		if (task.data.ref) branch = task.data.ref.replace('refs/heads/', '');
	...
	var buildJekyll = function (task, cb) {
		...
		exec('bundle exec jekyll build -d ' + dest_dir, {cwd: jekyll_dir}, function (err, stdout, stderr) {
			var result = '';
	...
	var runJekyll = function (task, cb) {
		buildJekyll(task, function (err, result) {
				if (!err) {
					...
					var params = '';
					if (err.indexOf('Could not locate Gemfile') >= 0) {
						params = '--gemfile=' + path.resolve(__dirname, '..', 'resource', 'Gemfile') + ' ';
						//console.log('using default Gemfile', params);
					}
					var jekyll_dir = path.resolve(site.build_path);
					emit('installing', task, {path: jekyll_dir});
					return exec('bundle install ' + params + '--path ' + jekyll_dir + '/.gem', {cwd: jekyll_dir}, function (err, stdout, stderr) {
							if (stderr && stderr.toString().length > 0) {
								err = stderr.toString();
								emit('error', task, err);
								return cb(err);
